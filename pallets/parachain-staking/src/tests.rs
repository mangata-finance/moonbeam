...
fn cannot_join_candidates_if_candidate() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_noop!(
				Stake::join_candidates(Origin::signed(1), 11u128, 1u32, 100u32, 10000u32),
...
fn cannot_join_candidates_if_delegator() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_noop!(
				Stake::join_candidates(Origin::signed(2), 10u128, 1u32, 1u32, 10000u32),
...
fn cannot_join_candidates_without_min_bond() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_noop!(
				Stake::join_candidates(Origin::signed(1), 9u128, 1u32, 100u32, 10000u32),
...
fn cannot_join_candidates_with_more_than_available_balance() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_noop!(
				Stake::join_candidates(Origin::signed(1), 501u128, 1u32, 100u32, 10000u32),
...
fn execute_leave_candidates_emits_event() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_ok!(Stake::schedule_leave_candidates(Origin::signed(1), 1u32));
			roll_to(10);
			assert_ok!(Stake::execute_leave_candidates(
				Origin::signed(1),
				1,
				10000u32
			));
...
fn adding_removing_staking_token_works() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_noop!(
				Stake::join_candidates(Origin::signed(3), 10u128, 3u32, 100u32, 10000u32),
				Error::<Test>::StakingLiquidityTokenNotListed
			);
			assert_noop!(
				Stake::join_candidates(Origin::signed(4), 10u128, 4u32, 100u32, 10000u32),
				Error::<Test>::StakingLiquidityTokenNotListed
			);
			assert_noop!(
				Stake::join_candidates(Origin::signed(5), 10u128, 5u32, 100u32, 10000u32),
				Error::<Test>::StakingLiquidityTokenNotListed
			);
			assert_noop!(
				Stake::join_candidates(Origin::signed(6), 10u128, 6u32, 100u32, 10000u32),
				Error::<Test>::StakingLiquidityTokenNotListed
			);
			assert_noop!(
				Stake::join_candidates(Origin::signed(7), 10u128, 7u32, 100u32, 10000u32),
				Error::<Test>::StakingLiquidityTokenNotListed
			);
			...
			assert_noop!(
				Stake::join_candidates(Origin::signed(10), 10u128, 3u32, 100u32, 10000u32),
				Error::<Test>::StakingLiquidityTokenNotListed
			);
